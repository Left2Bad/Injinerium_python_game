...
def init_level():
    ...
    global score, level_start_ms, defeat_active, defeat_start_ms, sabotage_scores, victory_saved
    ...
    hall_enter_frames = load_frames_range(HALL_DOOR_EXIT_PREFIX, 1, 19)
    hall_leave_frames = load_frames_range(HALL_DOOR_EXIT_PREFIX, 19, 1)
    kitchen_leave_frames = load_frames_range(KITCHEN_DOOR_ENTER_PREFIX, 0, 17)
    kitchen_enter_frames = load_frames_range(KITCHEN_DOOR_ENTER_PREFIX, 17, 0)
    ...
    door_left = Door(
        ...
        frames_leave=hall_leave_frames,
    ...
    door_right = Door(
        ...
        frames_leave=kitchen_leave_frames,
...
class Enemy:
    ...
    def start_transition(self, door_left, door_right):
        if self.room == "hall":
            ...
            self.next_room = "hall"
        if self.transition_from and self.transition_to:
            self.transition_from.play_leave()
            self.transition_to.play_enter()
            self.ghost = True
            self.phase = "to_point"
